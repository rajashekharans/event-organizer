            ...
            <form class="needs-validation" method="post"  action="/events/{{ $event->id }}" novalidate>
                ...
                <div class=" mb-3">
                    ...
                    >
                        ...
                        @foreach($eventTypes as $eventType)
                            <option
                                @if ($eventType->id == (old('event_type_id')?: $event->event_type_id)) selected @endif
                                value="{{ $eventType->id }}"
                            >
                                {{ $eventType->name }}
                            </option>
                ...
                <div class=" mb-3">
                    ...
                    >
                        @foreach($organizers as $organizer)
                            <option
                                @if(old("organizers")) {{ (in_array($organizer->id, old("organizers")) ? "selected":"") }}
                                @elseif ($event_organizers) {{ (in_array($organizer->id, $event_organizers) ? "selected":"") }}
                                @endif
                                value="{{ $organizer->id }}"
                            >
                                {{ $organizer->name }}
                ...
                <div class=" mb-3">
                    ...
                    >
                        ...
                        @foreach($venues as $venue)
                            <option
                                @if ($venue->id == (old('venue_id') ?: $event->venue_id)) selected @endif
                                value="{{ $venue->id }}"
                            >
                                {{ $venue->name }}
                            </option>
                ...
                <div class="mb-3">
                    ...
                    <input
                        ...
                        name="date_time"
                        placeholder=""
                        value="{{ old('date_time')?: $event->date_time->format('Y-m-d').'T'.$event->date_time->format('h:i') }}"
                        required
                    >
                    <div class="invalid-feedback">
                ...
                <button class="btn btn-primary btn-lg btn-block" type="submit">Edit Event</button>
            </form>
        </div>
    </div>
@endsection('content')
